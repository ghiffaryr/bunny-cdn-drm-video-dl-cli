...
class BunnyVideoDRM:
    ...
    def __init__(self, embed_url, referer=None, name='', path=''):
        self.embed_url = embed_url if embed_url else sys.exit(1)
        self.referer = referer if referer else embed_url
        ...
        try:
            self.server_id = re.search(r'https://video-(.*?)\.mediadelivery\.net', embed_page).group(1)
    ...
    def prepare_dl(self) -> str:
        ...
        def ping(time: int, paused: str, res: str):
            md5_hash = md5(f'{self.secret}_{self.context_id}_{time}_{paused}_{res}'.encode('utf8')).hexdigest()
            params = {
                'hash': md5_hash,
                'time': time,
                'paused': paused,
            ...
            self.session.get(f'https://video-{self.server_id}.mediadelivery.net/.drm/{self.context_id}/ping',
                             params=params,
                             headers=self.headers['ping|activate'])

        def activate():
            self.session.get(f'https://video-{self.server_id}.mediadelivery.net/.drm/{self.context_id}/activate',
                             headers=self.headers['ping|activate'])

        def main_playlist():
            params = {'contextId': self.context_id, 'secret': self.secret}
            response = self.session.get(f'https://iframe.mediadelivery.net/{self.guid}/playlist.drm',
                                        params=params,
        ...
        def video_playlist():
            ...
            self.session.get(f'https://iframe.mediadelivery.net/{self.guid}/{resolution}/video.drm',
                             params=params,
                             headers=self.headers['playlist'])
...
def main():
    parser = argparse.ArgumentParser(description='BunnyVideoDRM Downloader')
    parser.add_argument('--referer', default='', help='Referer URL (address of your webpage). Defaults to embed_url if not provided.')
    parser.add_argument('--embed_url', required=True, help='Embed URL')
    parser.add_argument('--name', default='', help='Override file name (without extension)')
    parser.add_argument('--path', default='', help='Override download path')
    args = parser.parse_args()

    referer = args.referer if args.referer else args.embed_url
